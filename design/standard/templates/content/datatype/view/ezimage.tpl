...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        <img src={$image.url|ezroot|ezserveruri} width="{$image.width}" height="{$image.height}" {if $hspace}hspace="{$hspace}"{/if} style="{$inline_style}" alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
        {if $href}</a>{/if}
    {/if}
